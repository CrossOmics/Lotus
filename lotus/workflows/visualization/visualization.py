...
def tsne(
    ...
    *,
    ...
    Parameters:
        ...
        truth_key (str | None): Key name for truth labels in adata.obs. Default: None
        output_dir (Path | None): Output directory. Default: None
        save (str | bool): Save filename or False to not save. Default: "_tsne.png"
        show (bool): Show the plot. Default: False
        compute_tsne (bool): Compute t-SNE embedding if not already computed. Default: True
        n_pcs (int | None): Number of PCs to use. Default: None
        use_rep (str | None): Representation to use. Default: None
        perplexity (float): Perplexity parameter. Default: 30
        random_state (int): Random seed. Default: 0
        early_exaggeration (float): Early exaggeration parameter. Default: 12
        learning_rate (float): Learning rate. Default: 1000
    """
...
def diffmap(
    ...
    *,
    ...
    Parameters:
        ...
        truth_key (str | None): Key name for truth labels in adata.obs. Default: None
        output_dir (Path | None): Output directory. Default: None
        save (str | bool): Save filename or False to not save. Default: "_diffmap.png"
        show (bool): Show the plot. Default: False
        compute_diffmap (bool): Compute diffusion map embedding if not already computed. Default: True
        n_comps (int): Number of components. Default: 15
        neighbors_key (str | None): Key for neighbors in adata.uns. Default: None
        random_state (int): Random seed. Default: 0
    """
...
def draw_graph(
    ...
    **kwargs,
    ...
    Parameters:
        ...
        truth_key (str | None): Key name for truth labels in adata.obs. Default: None
        output_dir (Path | None): Output directory. Default: None
        save (str | bool): Save filename or False to not save. Default: "_draw_graph.png"
        show (bool): Show the plot. Default: False
        compute_draw_graph (bool): Compute graph layout if not already computed. Default: True
        layout (str): Layout algorithm. Default: "fa"
        init_pos (str | bool | None): Initial position. Default: None
        root (int | None): Root node. Default: None
        random_state (int): Random seed. Default: 0
        neighbors_key (str | None): Key for neighbors in adata.uns. Default: None
...
def visualization(
    ...
    **kwargs,
    ...
    Parameters:
        adata (AnnData): AnnData object
        method (Literal["umap", "tsne", "diffmap", "draw_graph", "coremap"]): Visualization method. Default: "umap"
        cluster_key (str): Key name for cluster labels in adata.obs. Default: "cplearn"
        truth_key (str | None): Key name for truth labels in adata.obs. Default: None
        output_dir (Path | None): Output directory. Default: None
        save (str | bool | None): Save filename or False to not save. Default: None (method-specific)
        show (bool): Show the plot. Default: False
