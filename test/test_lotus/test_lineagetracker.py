...
from lotus.lineagetracker import visualizer as lineage_visualizer
from lotus.lineagetracker.models import LineageNode
...
def test_visualizer_node_roles_and_labels():
    root = LineageNode(
        lid="root-node",
        parents=[],
        creation_op="create",
    ...
    terminal = LineageNode(
        lid="terminal-node",
        parents=["child-node"],
        creation_op="copy",
        display_name="final_copy",
        shape=(40, 50),
    )

    children = lineage_visualizer._child_lookup(
        {root.lid: root, child.lid: child, terminal.lid: terminal}
    )

    assert lineage_visualizer._node_role(root, has_children=bool(children[root.lid])) == "input"
    assert lineage_visualizer._node_role(child, has_children=bool(children[child.lid])) == "intermediate"
    assert lineage_visualizer._node_role(terminal, has_children=bool(children[terminal.lid])) == "output"

    input_label = lineage_visualizer._data_node_label(root, root.lid, "input")
    output_label = lineage_visualizer._data_node_label(terminal, terminal.lid, "output")
    input_attrs = lineage_visualizer._data_node_attrs(root, "input")
    op_label = lineage_visualizer._operation_node_label(
        "preprocessing.normalize_total",
        {"target_sum": 1e4, "layer": "counts"},
        1,
    )

    assert "raw_input" in input_label
    assert "AnnData  100 x 50" in input_label
    assert "op: root" in input_label
    assert "output" in output_label
    assert "final_copy" in output_label
    assert "copy" in output_label
    assert ">normalize_total<" in op_label
    assert "target_sum=10000.0" in op_label
    assert "layer=counts" in op_label


def test_visualizer_output_edges_are_emphasized():
    output_edge = lineage_visualizer._parent_edge_attrs("output")
    intermediate_edge = lineage_visualizer._parent_edge_attrs("intermediate")
    root = LineageNode(
        lid="root-node",
        parents=[],
        creation_op="create",
    ...
    assert output_edge["color"] == "#111111"
    assert output_edge["style"] == "solid"
    assert float(output_edge["penwidth"]) > float(intermediate_edge["penwidth"])
    assert intermediate_edge["color"] == "#666666"
    assert input_attrs["shape"] == "box"
    assert input_attrs["fillcolor"] == "#DCFCE7"
    assert input_attrs["color"] == "#166534"
...
def test_lineagetracker():
    ...
    # when Graphviz is available, otherwise it should fail gracefully.
    assert not tracker.graph_file.exists(), "PNG should not exist yet"
    render_missing_pngs(tracker.root_dir)
    if lineage_visualizer._graphviz_available():
        ...
        assert not tracker.graph_file.exists()
        print("[OK] Missing Graphviz skipped PNG rendering gracefully")
